...
def test_request_payout_zero_total_settles_directly_without_calling_paypal(monkeypatch):
    """물품/청구 전체 반려(자동·수동)로 claim은 있는데 합계가 0원인 run — PayPal을
    부를 이유가 없다. Cloud Tasks enqueue·월간 캡 예약 전부 건너뛰고 곧장
    SETTLED로 종결하며, 반려 사유는 notify-settlement-complete로 청구자에게
    알린다(reconcile()의 성공 경로와 같은 태스크)."""
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        "get_claims_for_run",
        lambda rid: [
            {"claim_id": "clm_1", "recipient_id": "rcp_1"},
            {"claim_id": "clm_2", "recipient_id": "rcp_2"},
        ],
    )
    settled_claims = []
    monkeypatch.setattr(
    ...
    enqueued = []
    monkeypatch.setattr(
        routes, "enqueue_task", lambda path, body, **kw: enqueued.append((path, body))
    )

    # 아래가 호출되면 실패 — PayPal·Cloud Tasks 결제 경로를 아예 타면 안 된다.
    ...
    assert enqueued[0][0] == "/tasks/notify-settlement-complete"
    assert {r["recipient_id"] for r in enqueued[0][1]["recipients"]} == {"rcp_1", "rcp_2"}
    assert all(r["amount_minor"] == 0 for r in enqueued[0][1]["recipients"])
