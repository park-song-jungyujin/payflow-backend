...
- items: 영수증에 개별 품목·수량이 인쇄돼 있으면 한 줄씩 옮긴다. 각 항목은
  name(품목명)과 amount_text(그 줄의 금액, amount_text와 같은 규칙 — 계산·환산
  없이 보이는 그대로)로 구성한다. 품목별 금액이 없거나 합계만 찍혀 있으면
  amount_text는 비우고 name만 채운다. 품목 구분이 아예 안 되는 영수증(예: 택시비
  영수증)은 items를 빈 리스트로 둔다.
